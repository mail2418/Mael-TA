...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        for i, patchsize in enumerate(self.patch_size):
            self.embedding_patch_size.append(DataEmbedding(self.name, patchsize, configs.d_model, dropout=configs.dropout,decode=True))
            self.embedding_patch_num.append(DataEmbedding(self.name, self.win_size//patchsize,configs.d_model, dropout=configs.dropout, decode=True))

        self.embedding_window_size = DataEmbedding(self.name, configs.enc_in, configs.d_model, dropout=configs.dropout, decode=True)
