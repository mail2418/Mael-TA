...
class PSMSegLoader(object):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        self.train = data
        self.val = self.test
        self.test_labels = pd.read_csv(root_path + '/test_label.csv').values[:, 1:]
...
class MSLSegLoader(object):
    def __init__(self, root_path, win_size, step=1, flag="train"):
        ...
        self.win_size = win_size
        self.scaler = StandardScaler()
        data = np.load(root_path + "/MSL_train.npy")
        self.scaler.fit(data)
        data = self.scaler.transform(data)
        test_data = np.load(root_path + "/MSL_test.npy")
        self.test = self.scaler.transform(test_data)
        self.train = data
        self.val = self.test
        self.test_labels = np.load(root_path + "/MSL_test_label.npy")
